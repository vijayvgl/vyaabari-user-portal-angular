...
import { TgssNavigationItem, TgssVerticalNavigationAppearance, TgssVerticalNavigationMode, TgssVerticalNavigationPosition } from '@tgss/components/navigation/navigation.types';
...
@Component({
    selector: 'tgss-vertical-navigation',
    templateUrl: './vertical.component.html',
    styleUrls: ['./vertical.component.scss'],
    animations: tgssAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'tgssVerticalNavigation'
})
export class TgssVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy {
    ...
    @Input() appearance: TgssVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: TgssVerticalNavigationMode = 'side';
    @Input() name: string = this._tgssUtilsService.randomId();
    @Input() navigation: TgssNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: TgssVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output() readonly appearanceChanged: EventEmitter<TgssVerticalNavigationAppearance> = new EventEmitter<TgssVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<TgssVerticalNavigationMode> = new EventEmitter<TgssVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<TgssVerticalNavigationPosition> = new EventEmitter<TgssVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<TgssNavigationItem> = new ReplaySubject<TgssNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<TgssNavigationItem> = new ReplaySubject<TgssNavigationItem>(1);
    ...
    private _tgssScrollbarDirectives!: QueryList<TgssScrollbarDirective>;
    private _tgssScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'tgss-vertical-navigation-animations-enabled': this._animationsEnabled,
            [`tgss-vertical-navigation-appearance-${this.appearance}`]: true,
            // 'tgss-vertical-navigation-hover': this._hovered,
            'tgss-vertical-navigation-inner': this.inner,
            'tgss-vertical-navigation-mode-over': this.mode === 'over',
            'tgss-vertical-navigation-mode-side': this.mode === 'side',
            'tgss-vertical-navigation-opened': this.opened,
            'tgss-vertical-navigation-position-left': this.position === 'left',
            'tgss-vertical-navigation-position-right': this.position === 'right'
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    @HostBinding('style') get styleList(): any {
        return {
            'visibility': this.opened ? 'visible' : 'visible'
        };
    }

    /**
     * Setter for tgssScrollbarDirectives
     */
    @ViewChildren(TgssScrollbarDirective)
    set tgssScrollbarDirectives(tgssScrollbarDirectives: QueryList<TgssScrollbarDirective>) {
        // Store the directives
        this._tgssScrollbarDirectives = tgssScrollbarDirectives;

        // Return if there are no directives
        if (tgssScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._tgssScrollbarDirectivesSubscription) {
            this._tgssScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._tgssScrollbarDirectivesSubscription =
            merge(
                ...
                .subscribe(() => {
                    ...
                    tgssScrollbarDirectives.forEach((tgssScrollbarDirective) => {
                        tgssScrollbarDirective.update();
                    });
                });
    }
    ...
    ngAfterViewInit(): void {
        ...
        setTimeout(() => {
            ...
            if (!this._navigationContentEl.nativeElement.classList.contains('ps')) {
                ...
                const activeItem = this._navigationContentEl.nativeElement.querySelector('.tgss-vertical-navigation-item-active');
            ...
            // Otherwise
                ...
                this._tgssScrollbarDirectives.forEach((tgssScrollbarDirective) => {

                    // Skip if not enabled
                    if (!tgssScrollbarDirective.isEnabled()) {
                        return;
                    }

                    // Scroll to the active element
                    tgssScrollbarDirective.scrollToElement('.tgss-vertical-navigation-item-active', -120, true);
                });
            }
        });
    }
     ...
     * @param item
     */
    openAside(item: TgssNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: TgssNavigationItem): void {
        // Toggle
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('tgss-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add('tgss-vertical-navigation-overlay-transparent');
        }
    ...
    private _showAsideOverlay(): void {
        ...
        this._asideOverlay.classList.add('tgss-vertical-navigation-aside-overlay');
