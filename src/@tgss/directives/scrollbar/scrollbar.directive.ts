...
import { ScrollbarGeometry, ScrollbarPosition } from '@tgss/directives/scrollbar/scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
    selector: '[tgssScrollbar]',
    exportAs: 'tgssScrollbar'
})
export class TgssScrollbarDirective implements OnChanges, OnInit, OnDestroy
{
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_tgssScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() tgssScrollbar: boolean = true;
    @Input() tgssScrollbarOptions: PerfectScrollbar.Options;
    ...
    {
        // Enabled
        if ( 'tgssScrollbar' in changes )
        {
            // Interpret empty string as 'true'
            this.tgssScrollbar = coerceBooleanProperty(changes.tgssScrollbar.currentValue);

            // If enabled, init the directive
            if ( this.tgssScrollbar )
            {
                this._init();
            }
        ...
        // Scrollbar options
        if ( 'tgssScrollbarOptions' in changes )
        {
            // Merge the options
            this._options = merge({}, this._options, changes.tgssScrollbarOptions.currentValue);
